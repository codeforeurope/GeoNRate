...
location.getParam = function(name){
	...
	var results = regex.exec(this.search);
	if(results === null){
		return "";
	}
	else{
...
$(document).ready(function () {
	...
	} else if ($.url.attr("path").indexOf(conf.Url.ControllerActionAdd) !== - 1 ||
		$.url.attr("path").indexOf(conf.Url.ControllerActionStartup) !== - 1) {
		
		// add marker
		getMarkerId = 9999999;
		$('#map_wrapper_add').append('<div id="map"></div>');
		
	} else if ($.url.attr("path").indexOf(conf.Url.ControllerActionEdit) !== - 1 || 
		$.url.attr("path").indexOf(conf.Url.ControllerActionAdmin) !== - 1) {
	...
	} else if ($.url.attr("path").indexOf(conf.Url.ControllerActionMap) !== -1) {
...
if (getMarkerId == 9999999) {
	geojsonaddress=conf.masDir + 'markers/geojson'+ '/category:' + 
			categoryCond + '/status:' + statusCond

} else {
	geojsonaddress=conf.masDir + 'markers/geojson'+ slash + getMarkerId + '/category:' + 
			categoryCond + '/status:' + statusCond
}

	$.getJSON(geojsonaddress, function(data){	
				...
				if (!data && $('#map').not(':hidden')) {
					...
					if ($("#markersidebar")){
						...
						var html = "<p style=text-align:center>Nessun Hot Spot trovato</p>";
						li.innerHTML = html;
				...
				$.each(data, function(i, item) {
					...
					if (imageId) {
						htmlImg = '<span class="thumb"><a title="' + conf.Infwin.TabCommonLinkText +
							'" href="' + conf.masDir + 'markers/view/'+ id +'"><img src="'+ conf.masDir +'media/filter/s/'+
					...
					var html1 = '<span class="infomarker">';/*'<h4>Posizione attuale</h4>' + '<div id="newPos">'+ results[0].formatted_address + '</div>' + 
							'<div><a rel="external" href="' + conf.masDir + 'markers/startup/new/street:'+ Street	+
								'/zip:'+ Zip +'/city:'+ City +'">'+ conf.Infwin.TabCommonNewDescr +
									' </a></div>';*/
					var html2 = '<div>'+indirizzo[indicizzatore[id]].replace("'","\\'")+ " - " + citta[indicizzatore[id]].replace("'","\\'")+ '</div>';
//'<h4>' + conf.Infwin.TabCommonStatus + '</h4><div class="color_' +
						//item.properties.status.hex + '">' + item.properties.status.name +"</div>";
					var html3 = htmlImg + '<h4>' +conf.Infwin.TabCommonCategory +
						'</h4>' + '<div class="marker_kat">' + item.properties.category.name + '</div>';
					
				if (item.properties.category.name == 'HotSpot esistente') {
					...
					CloudMadeIcon.shadow = conf.masDir+"img/icons/cartosoft/marker_crts_shadow.png";
				...
				if (getToggle == 1) {					
					...
					CloudMadeIcon.image = conf.masDir+"img/icons/cartosoft/marker_crts_" + item.properties.category.hex + ".png";
					...
					if ($.cookie('CakeCookie[admin]')){	
						...
						editListener = function(marker){
							// update address when dragging
							CM.Event.addListener(marker, 'dragend', function(){
								addAddressToMap(marker.getLatLng(), false, false)
							});
						}
					} else {
				...
				if (getToggle == 2 || statusCond) {	
					CloudMadeIcon.image = conf.masDir+"img/icons/cartosoft/marker_crts_" + item.properties.status.hex + ".png";
					var markerOptions = {draggable:true,icon:CloudMadeIcon}; 
				}
				...
				if ($("#markersidebar")){
					var li = document.createElement('li');
					var html = "<a id="+ id +">"+ indirizzo[indicizzatore[id]] + " - " + citta[indicizzatore[id]] + "</a>";
					li.innerHTML = html;
				...
				if ((getMarkerId == 9999999) || (!getMarkerId)) {
		...
		//Initial point in "add" case
		if (getMarkerId == 9999999) {
			if (location.getParam('city') === "" && location.getParam('id') === "" && location.getParam('street') === "") {
				//geocoder.geocode({'address':$('#MarkerStreet').val() + ", " + $('#MarkerZip').val() + " " +
				//	$('#MarkerCity').val() + ", Italia"}, function(results, status) {
				//	addAddressToMap(results[0].geometry.location, false)
				//});
				geocoder.geocode({'address':conf.Basic.Street + ", " + conf.Basic.Zip + " " +
					conf.Basic.City  + ", Italia"}, function(results, status) {
					addAddressToMap(results[0].geometry.location, false);
				});
//				return;
			}
			else {
				geocoder.geocode({'address': + location.getParam('street') + ", " +
					location.getParam('city') + ", Italia"}, function(results, status) {
					addAddressToMap(results[0].geometry.location)
				});
//				return;
			}
	...
	// this was just wrong in my opinion
	//if (!$("#MarkerCity").val()) {
...
//			document.getElementById("marker_id").value=id;	
/*
...
//			alert('ciao');
	...
	function addAddressToMap(latlng, placeMarker, addListener) {
		// per chi verrà dopo: questo è un fix quick & dirty per far funzionare il tutto
		// sarebbe molto meglio riscrivere interamente tutto questo file
		// andrebbero fatte tante piccole funzioni riciclabili piuttosto che poche funzioni giganti in cui non ci si capisce una mazza
		// PS: il codice non l'ho scritto io! C'ho solo messo una pezza
		// by Federico
...
//						alert(results[0].formatted_address)
						...
						var quale=address.length-2;
						zipCity = address[quale].split(" ");
					} else {
				...
				if (address[address.length-1] !== "Italia") {
				...
				if (zipCity[1] !== conf.Basic.City && false) {
					alert("asdasd:"+conf.Text.NotTown);
				...
				*/
				...
				if(document.getElementById("n_spot")){
					if (n_hs>9) {
						document.getElementById("n_spot").style.color="red";
					} else {
						document.getElementById("n_spot").style.color="#888";
					}
					document.getElementById("n_spot").value=n_hs;  
				}
	
				var distanza;
				var distanza_minima=100;				
				var indirizzo_prossimo;
				var citta_prossimo;
				var tipo_prossimo=-1;
				for (var indice=0; indice<longitudine.length; indice++) {
					distanza=calcolaDistanza(latlng.lat(),latlng.lng(), latitudine[indice],longitudine[indice]);
					if (distanza<distanza_minima) {
						distanza_minima=distanza;
						indirizzo_prossimo=indirizzo[indice];
						citta_prossimo=citta[indice];
						tipo_prossimo=tipologia[indice];
					}
				}
				if(document.getElementById("distanza")){
					if (distanza_minima<100) {
						document.getElementById("distanza").style.color="red";
						if (tipo_prossimo==30) {
						document.getElementById("distanza").value='Hot spot esistente: '+indirizzo_prossimo+' '+citta_prossimo+' ('+distanza_minima+'m)';
						}
						else
						{
						document.getElementById("distanza").value='Esiste una proposta a: '+ indirizzo_prossimo+' '+citta_prossimo+' ('+distanza_minima+'m)';
						}
					}	
					else {
						document.getElementById("distanza").style.color="#888";
						document.getElementById("distanza").value='Nessuno';
					}  
				}
				// dovrà essere riscritto
				else{
					// if in edit marker view
...
//						map.setZoom(map.getZoom()+1);
					...
					map.panTo(point);
					if (getMarkerId != 9999999){ map.clearOverlays();}

					var CloudMadeIcon = new CM.Icon();
					CloudMadeIcon.iconSize = new CM.Size(32, 32);
					CloudMadeIcon.shadow = conf.masDir + "img/icons/cartosoft/marker_crts_shadow.png";
					CloudMadeIcon.shadowSize = new CM.Size(52, 35);
					CloudMadeIcon.image = conf.masDir + "img/icons/cartosoft/marker_crts_cccccc.png";

					markerOptions = { text:"Zieh mich!", draggable:true, icon:CloudMadeIcon };
					
					var marker;					
					var addMarkerToMap = function(latlng) {
						if(proposal !== undefined){
							map.removeOverlay(proposal);
						}
						proposal = new CM.Marker(latlng, markerOptions);
						// add marker to map
						map.addOverlay(proposal);						
						$('#map').css('cursor', 'default');
						// remove listener after having clicked (avoid subsequent clicks add more markers)
						//CM.Event.removeListener(map, 'click', addMarkerToMap);
						$('#MarkerStreet').val(Street);
						$('#MarkerZip').val(Zip);
						$('#MarkerCity').val(City);
						
						// update address after click
						addAddressToMap(latlng, false, false);
						
						// update address when dragging
						CM.Event.addListener(proposal, 'dragend', function(){
							addAddressToMap(proposal.getLatLng(), false, false)
						});
					}
					...
					else if(placeMarker === false && addListener == true){
						CM.Event.addListener(map, 'click', addMarkerToMap);	
					}
					
//					map.addOverlay(marker);
//					if ($("#mobile").length == 0){
//						marker.openInfoWindow('<h4>Posizione attuale</h4>' + '<div id="newPos">'+ results[0].formatted_address + '</div>'// + 
//						//	'<div><a rel="external" href="' + conf.masDir + 'markers/startup/new/street:'+ Street	+
//						//		'/zip:'+ Zip +'/city:'+ City +'">'+ conf.Infwin.TabCommonNewDescr +
//			//						' </a></div>'
//,{maxWidth:250});
//					}
				...
				if (saveId) {
					...
					if ($.cookie('CakeCookie[admin]')){	
						$.get(conf.masDir + 'markers/geosave/' + saveId + '/' + saveUrlAddress, function(data){
							$('#newPos').append('<span style="color:green">Posizione aggiornata.</span><br/>'+ 
								Street + ", " + Zip + " " + City + '</p>');
						});
					}
				}
				...
				if (getMarkerId == 9999999 && marker){
					map.panTo(point);
					//alert(map.getZoom());
					marker.openInfoWindow('<h4>Posizione attuale</h4>' + '<div id="newPos">'+ results[0].formatted_address + '</div>',{maxWidth:250});
...
*/
	...
	window.searchAddress = function(){
		geocoder.geocode({'address': $('#find_street').val()}, function(results, status) {
			//addAddressToMap(results[0].geometry.location, false, true);
			map.panTo(new CM.LatLng(results[0].geometry.location.lat(), results[0].geometry.location.lng()));
	...
	function resizeMap() {
		...
		var MasWidth = $('#content').width(); //$(window).width(); 
		...
		$('#map').css( {height: MasHeight-280, width: MasWidth} ); 
