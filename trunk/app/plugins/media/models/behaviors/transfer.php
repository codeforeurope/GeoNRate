	...
	var $_defaultSettings = array(
			...
			function checkLocation(&$Model, $field, $allow = true) {
				...
				foreach (array('source', 'temporary', 'destination') as $type) {
					...
					if ($type == 'source' && ${
						$type}['type'] == 'file-upload-remote') {
							continue;
						}
						if (!MediaValidation::location(${
							$type}['file'], $allow)) {
								return false;
							}
				}
			...
			function checkPermission(&$Model, $field, $match = true) {
				...
				foreach (array('source', 'temporary') as $type) {
					...
					if (!MediaValidation::permission(${
						$type}['permission'], $match)) {
							return false;
						}
				}
				return true;
			...
			function checkPixels(&$Model, $field, $max = false) {
				...
				foreach (array('source', 'temporary') as $type) { /* pixels value is optional */
					if (($type == 'temporary' && empty($$type)) || !isset(${
						$type}['pixels'])) {
							continue;
						}
						if (!MediaValidation::pixels(${
							$type}['pixels'], $max)) {
								return false;
							}
				}
			...
			function checkExtension(&$Model, $field, $deny = false, $allow = true) {
				...
				foreach (array('source', 'temporary', 'destination') as $type) {
					if (($type == 'temporary' && empty($$type)) || !isset(${
						...
						if (!MediaValidation::extension(${
							$type}['extension'], $deny, $allow)) {
								return false;
							}
				}
				return true;
			...
			function checkMimeType(&$Model, $field, $deny = false, $allow = true) {
				...
				foreach (array('source', 'temporary') as $type) {
					...
					*/
					...
					if (!isset(${
						$type}['mimeType']) && !$trustClient) {
							continue;
						}
						$result  = MediaValidation::mimeType(${
							$type}['mimeType'], $deny, $allow);
							$result |= MediaValidation::mimeType(Mime_Type::simplify(${
								$type}['mimeType']), $deny, $allow);
