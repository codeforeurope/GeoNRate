	...
	<div id="listIndex">
		...
		?>
			...
			<li><div class="color_<?php echo $marker['Status']['hex'] ?>">
				...
				<p class="status">Status: <?php echo $marker['Status']['name'] ?></p><p class="transactions"><?php __('This happened:') ?> <?php echo __($marker['Transaction'][0]['name'],true);?></p></div><small class="meta"><?php //echo $marker['User']['nickname']; ?><?php //__(', on ');
if ($transaction['Transaction']['created'] != Null) { 
	echo $datum->date_it($transaction['Transaction']['created'],1); 
}
?></small></li>
			<?php if ($i >= 20) {
			break;
	...
	<?php
		...
		<div class="logItem">
			...
			<div id="transaction_<?php echo $transaction['Transaction']['id']; ?>" title="<?php echo $transaction['Transaction']['id']; ?>">
				...
				<?php echo $html->link('ID: '.$text->truncate($transaction['Transaction']['marker_id'],60, array('ending' => '... ', 'exact' => false)), array('controller' => 'markers', 'action' => 'view', 'admin' => null, $transaction['Transaction']['marker_id']), array('escape'=>false)); ?><br/>
				<?php echo $transaction['Transaction']['Name'];	?>
				</p>

			 <small class="comment_meta"><span title="<?php //echo "IP ".$transaction['Transaction']['ip'];?>"><?php if ($transaction['Transaction']['created'] != Null) { echo $datum->date_it($transaction['Transaction']['created'],2); }?></span></small>
			</div>
						...
						for ($i = 0; $i <= 2; $i++) {
							if (!empty($attachments[$i]) && strstr($attachments[$i]['Attachment']['dirname'], 'img')) {
								echo '<div class="thumb">';
									echo '<a class="lightbox imageThumbView" href="'.$this->webroot.'media/filter/xl/'.$attachments[$i]['Attachment']['dirname']."/".substr($attachments[$i]['Attachment']['basename'],0,strlen($attachments[$i]['Attachment']['basename'])-3).'png">';
									
									echo '<img src ="'.$this->webroot.'media/filter/s/'.$attachments[$i]['Attachment']['dirname']."/".substr($attachments[$i]['Attachment']['basename'],0,strlen($attachments[$i]['Attachment']['basename'])-3).'png"/></a>
		...
		<?php
			...
			foreach ($comments as $comment):
				...
				?><div class="comment_admin"><a class="button small green comment_publish" id="publish_<?php echo $comment['Comment']['id']; ?>"href="#"><?php echo $linktext?></a> <a class="button small red comment_delete" id="delete_<?php echo $comment['Comment']['id']; ?>" href="#"><?php echo __('Delete', true);?></a></div>
					...
					<small class="comment_meta">Aggiunto da <?php echo $comment['Comment']['name'];?> il <?php echo $datum->date_it($comment['Comment']['created'],1);?></small>
				</div>
		<?php endforeach; ?>
		<?php endif; ?>
</div>	
