...
<?php		
	...
	<div id="content" class="page">
		...
		<div class="markers index">
			...
			<div id="listNoscript">
				...
				?>
						<li><img alt="<?php __('static map');?>" src="http://maps.google.com/staticmap?center=<?php echo $googleCenter?>&amp;zoom=12&amp;size=150x150&amp;maptype=mobile\&amp;markers=<?php echo $marker['Marker']['lat'].','.$marker['Marker']['lon']?>,blues%7C&amp;key=<?php echo $googleKey?>&amp;sensor=false" /><h3><?php echo $html->link($marker['Marker']['subject'], array('action' => 'view', $marker['Marker']['id'])); ?></h3><span><small><?php echo $marker['User']['nickname']; ?>, gemeldet: <?php echo $datum->date_it($marker['Marker']['created'],1);?></small></span>						<ul>
	...
	<!--	<div id="views"></div>
		...
		<?php if ($session->read('Auth.User.id') && $userGroup == $uGroupSysAdmin) {?>	
