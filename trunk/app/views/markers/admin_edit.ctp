	...
	<div id="content">
		...
		<?php if ($session->read('Auth.User.id')) {	?>
			...
			<div class="actions"><h4 class="hidden"><?php __('Actions');?></h4>
				<?php 
					...
					// gehoert der Marker diesem User?
					if ($marker['Marker']['user_id'] == $this->Session->read('username_caspur') || $userGroup == $uGroupAdmin || $userGroup == $uGroupSysAdmin) {	
		...
		<fieldset>
			...
			<?php 
				...
				echo '<div align="center"><input type="submit" value="'. __('Save information',true).'"></div>';
				echo '</p>';
	...
	<?php
		...
		<div class="log">
			...
			<div id="transaction_<?php echo $transaction['Transaction']['id']; ?>" title="<?php echo $transaction['Transaction']['id']; ?>">
			 ...
			 <small class="comment_meta"><span title="<?php //echo "IP ".$transaction['Transaction']['ip'];?>"><?php if ($transaction['Transaction']['created'] != Null) { echo $datum->date_it($transaction['Transaction']['created'],1); }?></span></small>
			</div>
		</div>
		<?php endforeach; ?>
	</div>
