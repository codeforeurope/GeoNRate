...
<div id="tabList">
	...
	<?php
	if ($session->check('Auth.User.id') && $userGroup == $uGroupSysAdmin) { ?>		
...
<?php
	...
	<tr id="sortUser">
		<th><?php echo $paginator->sort(__('Address',true), 'Marker.street');?></th>
...
<?php	} ?>
	...
	foreach ($markers as $marker):
		...
		<!--	<td class="votes">
			...
			<td class="votings">
				<?php //echo $marker['Marker']['rating']?>
				<?php  if ($marker['Marker']['category_id'] != 30) {
						echo $marker['Marker']['voting_pro'];
					} else {
						echo '-';
				}
				//echo $this->element('rating', array('plugin' => 'rating', 'model' => 'Marker', 'id' => $marker['Marker']['id'])); ?></td>
		<!--	<td class="votings">
				<?php //echo $marker['Marker']['rating']?>
			...
			<td class="date">
				<?php if ($marker['Marker']['modified'] != Null) {
					echo $datum->date_it($marker['Marker']['modified'],1);
					} ?>
 			</td>
			<?php if ($session->check('Auth.User.id')) {	?>
