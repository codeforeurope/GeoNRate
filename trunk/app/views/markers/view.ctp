...
<div id="content" title="<?php echo $marker['Marker']['id'];?>">
	...
	<div id="details">
		...
		<?php if ($session->read('Auth.User.id')): ?>
			...
			<div class="actions"><h4 class="hidden"><?php __('Actions');?></h4>
					...
					if ($marker['Marker']['user_id'] == $this->Session->read('username_caspur')):	
		...
		<?php endif; ?>
			...
			<dl class="color_<?php echo $marker['Status']['hex'];?>">
				...
				<dt class="<?php echo $marker['Category']['hex']; ?>">
	...
	<div id="descr_meta">
		...
		?></strong>--><?php if ($marker['Marker']['created'] != Null) { __('added: ');
			echo $datum->date_it($marker['Marker']['created'], 1) ?> | <?php
		 } if ($marker['Marker']['modified'] != Null) { __('last edited on');?> <?php echo $datum->date_it($marker['Marker']['modified'],1) ?> | <?php } echo __('Views:', true)." <strong>".$views."</strong>";?> | <?php echo __('Comments:', true)." <strong>".$commentSum."</strong>";?> 
</small>
	</div>
		...
		<?php if (!empty($marker['Marker']['feedback'])):;?>
			<?php if ($marker['Marker']['feedback'] == 2 || $marker['Marker']['feedback'] == 3) {?>
				<div id="ratings">
					<h3><?php __('Rating');?></h3>
			...
			<?php }?>
			<?php if (($marker['Marker']['feedback'] == 1 || $marker['Marker']['feedback'] == 3) && $marker['Marker']['category_id'] != 30 && $marker['Status']['id'] != 3 && $session->read('Auth.User.id') && $marker['Marker']['user_id'] != $this->Session->read('username_caspur')) {?>
				<div id="votings">
					<h3><?php __('Voting');?></h3>
		...
		<?php } else if ($marker['Marker']['user_id'] == $this->Session->read('username_caspur')) {?>
			<!-- this is probably not necessary <div align="center"><h3>Non puoi votare una tua proposta.</h3></div>-->
		<?php } else if (!$session->read('Auth.User.id') && $marker['Marker']['category_id'] != 30) {?>
			<div align="center"><h3>Per votare e commentare devi essere autenticato.</h3></div>
		<?php } ?>
	<?php endif; ?>
	...
	if ($marker['Marker']['category_id'] != 30 && $marker['Status']['id'] != 3) {
		...
		<?php
			...
			foreach ($marker['Attachment'] as $attachment) {
				...
				if ($attachment['dirname'] == "img") {
					...
					echo '<a class="lightbox imageThumbView" href="'.$this->webroot.'media/filter/xl/'.$attachment['dirname']."/".substr($attachment['basename'],0,strlen($attachment['basename'])-3).'png"><img src ="'.$this->webroot.'media/filter/m/'.$attachment['dirname']."/".substr($attachment['basename'],0,strlen($attachment['basename'])-3).'png"/></a></div>';
				} elseif ($attachment['dirname'] == "doc"){
					echo '<div class="doc"><a href="'.$this->webroot.'media/transfer/doc/'.$attachment['basename'].'">'.$attachment['basename'].'</a></div>';
				} else {
	...
	<?php
		...
		<div class="log">
			...
			<p title="<?php echo $thisColors[$thisStatus['Status']][1];?>" class="color_<?php echo $thisColors[$thisStatus['Status']][0];?>">
			 ...
			 <small class="comment_meta"><span title="<?php //echo "IP ".$transaction['Transaction']['ip'];?>"><?php echo $datum->date_it($transaction['Transaction']['created'],1);?></span></small>
		</div>
		<?php endforeach; ?>
	</div>
