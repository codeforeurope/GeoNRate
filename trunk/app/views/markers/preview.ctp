...
<div id="content">
	...
	<div id="descr_meta">
		<small><?php __('added: '); ?> <?php echo $datum->date_it($marker['Marker']['created']) ?> <?php //__('by '); ?>
		<?php 
		...
		?>
		 | <?php __('last edited on'); ?>	<?php echo $datum->date_it($marker['Marker']['modified']) ?></small>
	</div>
	<!--<div id="ratings">
		<h3><?php __('Rating');?></h3>
		...
		<?php
			...
			foreach ($marker['Attachment'] as $attachment) {
				...
				if ($attachment['dirname'] == "img") {
					...
					echo '<a class="lightbox imageThumbView" href="'.$this->webroot.'media/filter/xl/'.$attachment['dirname']."/".substr($attachment['basename'],0,strlen($attachment['basename'])-3).'png"><img src ="media/filter/m/'.$attachment['dirname']."/".substr($attachment['basename'],0,strlen($attachment['basename'])-3).'png"/></a></div>';
